...
overlays** (FR-OVL-2). These expand coverage but do not block the gate; if they slip,
v1.0 still ships a complete closed loop on React/Next + agnostic HTML.
...
- **v1.0 Release Gate (Must — blocks release):** closed loop on web for **React/Next +
  ...
  capture backend** (static+screenshot fallback), GitHub Issues export, explain mode.
- **v1.0 Committed (non-gating — in v1.0 if ready, else v1.1):** **Vue + Svelte adapters**,
  the **second capture backend** (both Playwright and agent-browser), and **SaaS /
  e-commerce / marketing overlays**.
